...
class MassiveSeederCommand extends Command
{
	...
	public function handle()
	{
		...
		$batchSize = 1000;
		$totalRecords = text(
			label: 'How many records do you want to insert? ([min: 1,000] [max: 1,000,000])',
			validate: ['name' => 'required|min:1000|max:1000000|integer'],
		);
		...
		$this->info('End generating tasks...');
		$bar->start();
		$results = Fork::new()
					   ->after(
	...
	public static function allTablesMySQL($connection = null)
	{
		...
		if (!$database) {
			throw new \Exception('Database not found in the connection configuration.');
		}
